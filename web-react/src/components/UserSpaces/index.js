...
function UserSpaces(props) {
  ...
  const handleGetStartedClick = async () => {
    ...
    if (result.error) {
      ...
      // Success
      if (state.refScreen === "USER_PROFILE") {
  ...
  useEffect(() => {
    ...
    setState((state) => ({
      ...state,
      refScreen: queryString.parse(props.location.search).ref,
    }));
  }, []);
      ...
      <main className={cx(styles.mainContainer, "globalContainer")}>
        <section className={cx(styles.spacesContainer)}>
          ...
          <Spaces
            ...
            onClick={(_id, selected) => handleSpaceClick(_id, selected)}
          />
          {state.refScreen === "USER_PROFILE" && (
            <Button
              shape="round"
              htmlType="submit"
              color="secondary"
              disabled={state.loading || state.selectedSpaces.length === 0}
